...
async def cluster_logs_download_url(
    ...
    Returns:
        str: URL for downloading cluster logs.
    """
    ...
    try:
        ...
        return f"Failed to retrieve logs URL for cluster {cluster_id}: {str(e)}"

    if not logs_url:
        log.warning("No logs URL available for cluster %s", cluster_id)
        return f"No logs URL available for cluster {cluster_id}"
